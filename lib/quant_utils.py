...
def unpack_i4(x: torch.Tensor):
    ...
    out = torch.empty(out_shape, device=DEV, dtype=torch.int32)
...
class ActQuantWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        device = DEV  # Ensure device consistency
        x_dtype = x.dtype

        # Rotate, if needed
        if self.online_full_had:
            if self.fp32_had:  # Full Hadamard in FP32
                x = matmul_hadU_cuda(x.float().to(device), self.had_K.to(device), self.K).to(x_dtype)
            else:  # Full Hadamard in FP16
                x = matmul_hadU_cuda(x.to(device), self.had_K.to(device), self.K)
            ...
            init_shape = x.shape
            if self.K == 1:
                x = fast_hadamard_transform.hadamard_transform(x.reshape(-1, init_shape[-1] // self.had_dim, self.had_dim).transpose(1, 2).to(device),
                                                               scale=1 / math.sqrt(init_shape[-1] // self.had_dim)).transpose(1, 2)
            else:
                x = (self.had_K.to(x.dtype).to(device) @ x.reshape(-1, init_shape[-1] // self.had_dim, self.had_dim).to(device)) / math.sqrt(init_shape[-1] // self.had_dim)
        ...
        if self.quantizer.bits < 16:  # Quantize, if needed
            self.quantizer.find_params(x.to(device))
            x = self.quantizer(x.to(device)).to(x_dtype)
            self.quantizer.free()

        x = self.module(x.to(device)).to(x_dtype)

        if self.out_quantizer.bits < 16:  # Quantize the output, if needed
            self.out_quantizer.find_params(x.to(device))
            x = self.out_quantizer(x.to(device)).to(x_dtype)
